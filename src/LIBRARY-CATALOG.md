...
| `BlazorAtoms.Typography` *(in progress)* | AtomTextCycle *(shipped)* — zero-JS vertical flip-cascade word rotator, cycles `Words` in an infinite loop; per-instance-generated `@keyframes` sized to word count, duplicate-first-word row for a seamless wrap. Still to build: AtomHeading, AtomText, AtomCode, AtomKbd, AtomTruncate | JS-free |
...
- **Typography** — text primitives. `AtomTextCycle` (shipped) cycles a list of words/phrases in an
  infinite vertical flip-cascade loop, zero JS. Still to build: headings, body text, inline code,
  keyboard keys, truncation.
