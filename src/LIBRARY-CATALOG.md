...
| `BlazorAtoms.Highlights` *(implemented)* | AtomHighlight — zero-JS highlighter for plain-text child content; renders `<mark>` during Blazor render, works in every render mode · AtomHighlightDeep — full DOM text-node highlighter for arbitrary/unknown child content (nested components, mixed markup), similar to `jquery.highlight`; tiny self-loaded JS module. Both share `HighlightStyle` (`Mark`/`Underline`/`Outline`) and CSS variables. |
