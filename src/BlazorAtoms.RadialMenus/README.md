...
@* Icon-only, always open, with spokes back to the hub *@
<AtomRadialMenu Items="@items"
...
@* Deep tree in a bounded box: children replace the ring, center becomes Back *@
...
| `ExpandMode` | Behaviour |
|---|---|
| `Cascade` **(default)** | children radiate from the branch item, on an arc centred on the direction it already points |
| `Concentric` | children go on the next ring out from the same center, confined to the parent's slice |
| `Drill` | children replace the ring; the center button becomes Back. Same footprint at any depth |
...
`Debug="true"` draws the arc bounds, a tick and an angle/radius tag per item, the ring circles, and
lists anything the layout had to compromise on — a crowded ring, a `Fixed` radius that overlaps, a
`Shrink` that hit `MinItemSize`, `Endpoints` on a closed arc. Advisories are never exceptions; the
