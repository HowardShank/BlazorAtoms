...
public class AtomBadgeTests : BunitContext
{
    ...
    {
        var cut = Render<AtomBadge>(p => p.Add(c => c.Value, null));
    ...
    {
        var cut = Render<AtomBadge>(p => p.Add(c => c.Value, ""));
        Assert.Empty(cut.FindAll(".atom-badge"));
    }
    ...
    {
        var cut = Render<AtomBadge>(p => p.Add(c => c.Value, "NEW"));
    ...
    {
        var hidden = Render<AtomBadge>(p => p.Add(c => c.Value, 0));
        Assert.Empty(hidden.FindAll(".atom-badge"));

        var shown = Render<AtomBadge>(p => p
            .Add(c => c.Value, 0)
    ...
    {
        var cut = Render<AtomBadge>(p => p.Add(c => c.Value, Priority.High));
    ...
    {
        var cut = Render<AtomBadge>(p => p.Add(c => c.Value, 3));
