...
public class AtomInitialsAvatarTests : TestContext
{
    [Fact]
    public void Derives_initials_from_two_word_name()
    {
        var cut = RenderComponent<AtomInitialsAvatar>(p => p.Add(c => c.Name, "Ada Lovelace"));
    ...
    {
        var cut = RenderComponent<AtomInitialsAvatar>(p => p
    ...
    {
        string Bg(string name) => RenderComponent<AtomInitialsAvatar>(p => p.Add(c => c.Name, name))
